...
async function auditOpenDialog(page: Page, testInfo: TestInfo, label: string): Promise<void> {
  const dialog = page.locator("[role='dialog'], dialog[open], [aria-modal='true']").filter({ visible: true }).first();
  await expect(dialog, `${label} must open a visible dialog/sheet`).toBeVisible();
