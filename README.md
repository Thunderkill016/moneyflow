...
Historical research remains useful as evidence but does not override current code, the current-project memory or accepted owner decisions. Every pull request targeting `main` must create one truthful bounded record in `docs/research/pr-memory/YYYY/QN/`. A PR that changes implementation status must also update the affected row or section in the current-project memory.
...
8. Create the PR's bounded record at `docs/research/pr-memory/YYYY/QN/PR-<number>.md`; update `CURRENT_PROJECT_MEMORY.md` too when implementation status changed.
9. Open or update the pull request and evaluate the actual diff against the stated scope.
