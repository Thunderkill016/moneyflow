...
function enforcePullRequestMemoryUpdate() {
  ...
  try {
    ...
    const statusImpact = record.match(/^- Status impact:\s*(.+)$/mu)?.[1]?.trim() ?? "";
    const snapshotUpdate = record.match(/^- Snapshot update:\s*(.+)$/mu)?.[1]?.trim() ?? "";
    const snapshotChanged = changedFiles.includes("docs/research/CURRENT_PROJECT_MEMORY.md");
...
try {
  ...
  for (const link of requiredReadmeLinks) if (!readme.includes(link)) failures.push(`README.md must link to ${link}`);
} catch {
  // Missing file already reported.
}
...
for (const warning of warnings) console.warn(`Project knowledge warning: ${warning}`);
