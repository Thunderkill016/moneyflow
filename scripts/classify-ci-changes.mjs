...
export const workflowOrPolicyMatchers = [
  ...
  equals("scripts/check-project-knowledge.mjs"),
  equals("scripts/project-knowledge-contract.mjs"),
  equals("scripts/project-knowledge-contract.test.mjs"),
  equals("scripts/active-packet-registry.mjs"),
  equals("scripts/agent-doctor.mjs"),
  equals("scripts/agent-doctor.test.mjs"),
  equals("docs/research/PROJECT_KNOWLEDGE_CONTRACT.json"),
];
...
const browserSmokeMatchers = [
  startsWith("src/"), startsWith("e2e/"), startsWith("public/"), startsWith("tests/"),
  matches(/^playwright(?:\..+)?\.config\.[cm]?[jt]s$/), matches(/^next\.config\.[cm]?[jt]s$/),
  isRequestBoundary, equals("package.json"), equals("package-lock.json"),
  equals(".github/workflows/ci.yml"), equals("scripts/classify-ci-changes.mjs"),
  equals("scripts/classify-ci-changes.test.mjs"),
];

const uiAuditMatchers = [
  matches(/^src\/app\/.*\.(?:tsx|css)$/), matches(/^src\/components\/.*\.(?:tsx|css)$/),
  startsWith("src/lib/demo/"), startsWith("public/"), startsWith("e2e/audit/"),
  matches(/^src\/.*\.css$/), equals("playwright.audit.config.ts"),
  equals(".github/workflows/ci.yml"), equals("scripts/classify-ci-changes.mjs"),
  equals("scripts/classify-ci-changes.test.mjs"),
];

const codeqlMatchers = [
  isJavaScriptOrTypeScript, equals("package.json"), equals("package-lock.json"),
  equals("eslint.config.mjs"), equals("tsconfig.json"), equals("next.config.ts"),
  equals(".github/workflows/codeql.yml"), equals(".github/workflows/ci.yml"),
];
...
export function classifyChanges(inputFiles, { forceFull = false } = {}) {
  ...
  if (forceFull || files.length === 0) {
    return {
      files, docsOnly: false, fullVerify: true, database: true, browserSmoke: true,
      uiAudit: true, codeql: true,
  ...
  const pureDatabaseOnly = hasDatabaseChange && files.every(
    (file) => isDocumentationOnlyPath(file) || matchesAny(file, pureDatabaseMatchers),
  );
  if (workflowOrPolicy) {
    return {
      files, docsOnly: false, fullVerify: true, database: true, browserSmoke: true,
      uiAudit: true, codeql: true, reason: "CI workflow or policy changed; exercise every gate",
    };
  }
  return {
    files,
    ...
    reason: docsOnly ? "documentation-only change" : pureDatabaseOnly ? "database-only change" : "risk-proportional checks selected from changed paths",
  };
}

function toOutputEntries(result) {
  return {
    docs_only: String(result.docsOnly), full_verify: String(result.fullVerify),
    database: String(result.database), browser_smoke: String(result.browserSmoke),
    ui_audit: String(result.uiAudit), codeql: String(result.codeql), reason: result.reason,
