...
function patchInboxMap(source: string): string {
  source = replaceOnce(
    source,
    `import {
  createCandidate,
...
function patchServerInbox(source: string): string {
  source = replaceOnce(
    source,
  ...
  source = replaceOnce(
    ...
    "server_batch_columns",
  );
  return source;
}

function patchInboxActions(source: string): string {
  source = replaceOnce(
    source,
...
function patchPlanCount(source: string): string {
  return replaceOnce(source, "select plan(38);", "select plan(39);", "plan_count");
}
...
const outputs = {
  ...
  "supabase/tests/database/import_provenance_invariants.test.sql": patchPlanCount(
    readFileSync("supabase/tests/database/import_provenance_invariants.test.sql", "utf8"),
  ),
};
