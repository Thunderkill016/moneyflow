...
| A3 | Capability coverage expansion theo nhu cầu product: `accounts.list`, `budgets.status`, `goals.status`, `activity.candidates` — mỗi cái giờ là schema + injectable deps + golden + manifest regen (giá rẻ); chỉ thêm khi product surface cần, không add speculatively | in PR | 1 mỗi cái |
