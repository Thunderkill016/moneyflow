...
- [ ] **#460 P2 — remembered Direct CSV column mapping** — the one active agent slice. A user can explicitly remember a confirmed mapping for a normalized CSV header shape on the same device, then choose it on a later matching import before the existing dry-run/commit. It stores no rows, filenames, raw snippets, account/category choice or financial values; invalid/stale presets fail closed to ordinary auto-map. No automatic approval or ledger/source/RPC/schema/RLS/provider change. **Done when:** matching repeat imports can reuse the explicit local preset, mismatched/malformed headers never apply it, and all existing import review/commit semantics are unchanged. **Next actor:** agent.
...
PR #459 completed #458: a retained Direct CSV approval failure now directs the user to existing Inbox/import-history review without retry, approval, ledger/source mutation, RPC/schema/RLS, provider or deployment change.

## NEXT

- [ ] **#432 P2 — Low-Maintenance Ingestion** — after #460, select the next bounded evidence-backed improvement across real Vietnamese file/share sources, normalization or exception-first review; reduce interventions/100 transactions without reducing match precision. Do not promote work without its bounded issue/spec/packet.
...
| `460-direct-csv-mapping-presets.md` | current agent-executable P2 implementation | browser-local explicit column-mapping reuse only; no financial/source/provider mutation |
