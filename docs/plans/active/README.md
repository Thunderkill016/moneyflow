...
**Current main baseline:** `361a6e351062fe690bb09e6189dfd53f2226d6da` (#406 merged)
**Release readiness:** **NOT PUBLIC-BETA READY** — RRB-01 and RRB-07 are closed with current browser/runtime evidence. RRB-08 real-phone observation has started and exposed bounded P2 mobile findings; #404 merged the keyboard-pressure implementation, #406 merged a local `Ghi` spacing correction, and owner real-phone review rejected the remaining protruding center-action architecture. #407 is now the owner-authorized redesign of the entire mobile bottom navigation. RRB-08 still requires owner physical-device re-test/closeout. Remaining P1 release gates still depend on owner/provider/legal/read-access.
...
- [ ] **#407 — redesign mobile bottom navigation as a unified five-slot action dock** — owner-authorized after real-phone review of #406 showed that the center `Ghi` control still protrudes too aggressively and the whole bar reads as a patched FAB/tab hybrid. **Current scope:** redesign the entire five-slot presentation while preserving `Tổng quan / Giao dịch / Ghi / Tài khoản / Thêm`, current route/action semantics, safe-area ownership, touch-safe targets and Fresh Blue identity. Use one shared icon-row + label-row geometry; emphasize `Ghi` by filled brand shape inside the dock, not by protrusion/translate/pseudo-FAB hacks. **Done when:** focused implementation + regression contract + responsive/browser evidence are ready in one PR; physical-phone acceptance remains owner-observed. **Next actor:** agent.
- [ ] **RRB-08 — current physical-device proof** — issue #398. The bounded runbook/evidence contract is merged in #399 as `rrb-08-physical-device-proof.md`. Owner-observed real-phone testing has started; #404 addressed keyboard pressure, #406 documented and attempted the first `Ghi` spacing correction, and #407 now owns the broader bottom-nav redesign revealed by direct phone review. **Done when:** after the current bounded fixes, a real phone completes/repeats the release-critical smoke and records device/OS/browser/origin/mode, pass/fail and observed defects. Browser/emulation does not satisfy the final physical-device verdict. **Next actor:** owner after #407 is deployed/available for re-test.

## NEXT

- [ ] **#403 — canonical page-load / dashboard LCP optimization** — explicitly authorized by the owner but held behind the one-at-a-time queue. Promote only after #407 reaches reviewed PR/owner handoff, merges or is explicitly stopped. Measure current `/` and `/dashboard` before/after; do not change private financial caching semantics. **Next actor:** agent after #407.
