...
- [x] **#440 / PR #441** — same stable source ID with changed evidence is now a distinct hard source plan; reviewed resolution preserves the newer source observation without rewriting ledger/reconciliation/canonical provenance. Approved observations are browser-immutable, including direct INSERT/UPDATE forgery protection. Final head `237aac8d…` passed CI #2792, CodeQL #1851 and Secret history #1851 with no retry before squash merge as `6123d263…`.
- [x] **#438 / PR #439** — deleted exact-source reimport is explicit: live same-ID remains hard duplicate; deleted unchanged evidence can be reviewed to restore the same transaction; deleted changed evidence stays blocked. Final head `00384172…` passed CI #2766, CodeQL #1826 and Secret history #1826 before squash merge as `d5324c47…`.
...
- [x] **#432 P0 / PR #433** — acquisition-first long-term direction, target architecture, reference-repo atlas and program tracking merged as `a35d6f96…`.
