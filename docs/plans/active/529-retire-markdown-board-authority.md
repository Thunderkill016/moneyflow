...
Current executable authority was duplicated across `PLAN_AUTHORITY.json`, a Markdown board table, manual SHA baseline/projection markers, packet metadata and project memory. The failure is caused by duplicated state, not application runtime behavior.
...
Internal evidence is stronger for the machine boundary: the manifest already owned master-plan supersession and Git first-parent history already proves merged introduction PRs. Extending that manifest to own `current` removes the hand-maintained commit-baseline race.
...
- Replace board table with retired-pointer content and ensure validation prevents it regaining authority semantics.
...
| G2 | Move master/current executable authority into manifest schema v2 | done |
| G3 | Remove board baseline/projection parsing from resolver/lifecycle | done |
| G4 | Replace board with compatibility tombstone and update docs/memory | done |
| G5 | Run exact-head CI and repair only migration defects | in progress |
