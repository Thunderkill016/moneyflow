...
- `AppShell` defines mobile-nav reserve custom properties only under `html[data-moneyflow-shell="mounted"]`, while the attribute is added in a client `useEffect`. This leaves a real first-paint interval in which fixed mobile navigation can render before the shell reserve variables exist.
...
- Phase 5–10 packets and `CURRENT_PROJECT_MEMORY.md` are stale relative to merged truth.
...
| `src/components/layout/app-shell.tsx` | Currently adds shell root marker after hydration | preserve unless marker lifecycle still has a separate purpose |
| `e2e/audit/critical-browser.audit.spec.ts` | Contains the flaky mobile-shell geometry assertion | strengthen after implementation fix |
| `docs/plans/active/ui-phase-5-*.md` through `ui-phase-10-*.md` | Stale lifecycle records after successful merges | reconcile to merged/deployed truth |
...
- [x] Is the WebKit retry merely a harmless assertion race? **No.** Repository source shows the underlying safe-area/nav reserve variables are created only after `useEffect`, so first-paint overlap is possible.
- [ ] Is any physical Android device/AVD connected to the current execution environment?
...
- Expected decision: distinguish real-device evidence from emulation and remove non-retrying timing assertions where the page state is asynchronous.
...
| Playwright Android / Emulation | official docs | 2026-08-08 | emulation simulates device parameters; Android real-device automation requires a device/AVD and ADB | does not provide physical iOS Safari automation for this environment |
...
| Change only the test to `expect.poll` | removes timing flake | could mask real initial overlap | reject as sole fix |
| Make shell reserve variables available on first paint, then use retrying measurement for stabilized geometry | fixes product and makes test deterministic | requires narrow shell CSS/test change | selected |
| Add a new browser/device cloud provider | could supply real-device evidence | new cost/secrets/provider boundary; not needed to fix current source issue | reject for P11 branch |

### Research decision

Fix the product ownership first: mobile-nav reserve variables must not depend on a post-hydration effect. Keep the root marker only for concerns that truly require mounted-shell lifecycle. Then make complex geometry evidence retry until the stabilized CSS state is observable instead of taking one non-retrying snapshot. Physical-device acceptance remains a separate gate and will not be inferred from browser emulation.
...
- [ ] **P11-AC4:** physical Android Chrome smoke is executed on a maintained real device/AVD connected through ADB and evidence is recorded.
...
- [ ] **P11-AC7:** Phase 5–10 packets and current project memory reflect actual merges/deployments and do not mark P11/program acceptance before AC1–AC6 are satisfied.
...
`AppShell` already owns mobile navigation geometry. The fix stays in that owner: define layout-critical custom properties where `.shell` can consume them on server-rendered first paint, while retaining the mounted root marker only for document-level scroll/focus behavior. Browser evidence remains in the existing critical audit rather than adding a parallel test harness.

### Planned changes

| File/area | Change | Reason |
|---|---|---|
| `src/components/layout/app-shell.module.css` | move first-paint layout-critical nav reserve variables onto `.shell`/stable CSS ownership; preserve document-level marker rules | eliminate hydration gap |
| `e2e/audit/critical-browser.audit.spec.ts` | poll stabilized geometry after visible nav, while still failing if reserve never becomes correct | deterministic semantic evidence |
| Phase 5–10 packets | reconcile delivered/merged state and evidence | remove false active work |
| `CURRENT_PROJECT_MEMORY.md` | record current main/P0–P10/P11 truth | durable handoff |
...
|---|---|
| CSS variable move changes desktop | scope reserve variable consumption to existing shell/mobile media rules; full matrix |
| Test polling hides a permanent product defect | poll the actual geometry invariant with timeout; exact value must eventually satisfy invariant |
| Records say `completed` although physical gates are missing | only P5–P10 become completed; P11 remains implementing/blocked |
| Emulation is mistaken for physical device | explicit AC4/AC5 and issue #72 remain open |
...
| P11-T3 | Execute physical Android Chrome smoke | P11-T2 | device/ADB evidence + checklist | blocked until device exists |
...
| P11-T6 | Reconcile P5–P10 lifecycle records and current memory | verified merge/deploy truth | docs diff | planned |
...
|---|---|---|
| P11-AC1 | P10 artifact exposed zero-padding first attempt; fix pending | pending |
| P11-AC2 | P10 baseline: 426 pass / 127 skip / 1 flaky; P11 rerun pending | fail baseline / pending fix |
| P11-AC3 | P10 artifact downloaded; P11 post-fix review pending | pending |
| P11-AC4 | no physical Android evidence yet | blocked |
| P11-AC5 | no physical iOS evidence/waiver yet | blocked |
| P11-AC6 | public production read-only smoke green; dependency-ordered full journey not yet allowed | pending |
| P11-AC7 | stale P5–P10/current memory identified | pending reconciliation |
| P11-AC8 | P11 not accepted | blocked |

### Remaining limitations

- Browser emulation does not satisfy the physical-device gate.
- Current execution environment has not yet proven an attached Android device/AVD.
