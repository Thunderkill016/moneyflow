...
| Money renders with `tabular-nums` (`src/components/money-value.module.css:13-15`), and a sign appears only in `mode="kind"` via `formatMoneyWithKind` (`src/lib/money.ts:51-55`) | `src/components/money-value.tsx` | legibility of the sign is checkable without relying on colour, but only where the kind mode is used |
