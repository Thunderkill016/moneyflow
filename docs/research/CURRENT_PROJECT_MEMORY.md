...
- **Missions 17B and 17C are complete:** 17C deployed the Recover schema; hosted acceptance has not run.
...
| P2 Recover | **schema deployed, acceptance open** — all code merged and CI-proven, and the two migrations are now live in production; **no hosted backup or restore acceptance has run**, so P2 is not accepted |
...
- **the surface is inert until the two archive migrations are applied to production**: an absent function is reported as a capability gap rather than an error;
