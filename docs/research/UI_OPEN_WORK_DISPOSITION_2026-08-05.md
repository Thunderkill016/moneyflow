...
**Status:** Phase 0 recommendation — no PR or issue state change authorized  
**Baseline:** `main@9f31aa02a64bcff30705c187fceb09cf5fa61ded`  
**Parent program:** `docs/plans/active/ui-system-migration.md`

This table separates current project truth from candidate branches. It recommends the next decision but does not close, retarget, merge or modify any existing PR or issue.

## Open work

| Item | Current state | Relevant evidence | Phase 0 recommendation | Owner action required |
|---|---|---|---|---|
| PR #296 — UI system migration parent plan | Draft, mergeable, docs only | Defines eleven gated phases and explicit owner checkpoints | Keep. Review and merge as the program governance packet after corrections | Approve/revise parent packet |
...
| PR #295 — publishable-key fingerprint allowlist | Open, mergeable, security boundary | Addresses all-ref secret-scan blocker, not interface architecture | Handle independently under its Class 3 security packet. Do not bundle with UI work | Security review/merge decision |
...
| PR #170 — retire mobile chrome CSS | Open, old baseline | Valuable DOM measurements and proof that tests guarded dead selectors; reports double clearance | Do not merge wholesale. Reuse measured failure modes and before/after method | Close/supersede after Phase 0 records evidence and current slices replace it |
...
1. Merge or otherwise establish the parent governance packet (#296) after owner review.
2. Keep Phase 0 baseline PR independent and documentation-only.
3. Resolve PR #295 separately so all-ref secret scanning is trustworthy.
...
8. Close obsolete identity PR #119.
