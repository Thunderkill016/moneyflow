...
|---|---|---|---|
| Statement/history available | **confirmed** — VCB Digibank history workflow | **confirmed** — ACB ONE standard KHCN transaction history plus supported store-management flow | **confirmed** — iPay Web account history/statement plus current account/card material |
| Downloadable artifact | **confirmed: Excel** in VCB Digibank history workflow; `.xls` vs `.xlsx` not established | **confirmed: Excel** — ACB ONE standard KHCN guide documents `Xuất file excel` for transaction listing; `.xls` vs `.xlsx` not established | **confirmed: Excel** — VietinBank customer-support guidance says iPay Web can retrieve Excel detailed transaction data for the account; `.xls` vs `.xlsx` not established |
| Exact exported headers/layout | **unknown** | **unknown** — consumer Excel availability is confirmed, but exact exported columns/layout are not | **unknown** |
...
| Transaction reference | **observed-but-unverified/display-only** — UI shows `Số tham chiếu`; cross-export stability unproven | **unknown** — standard KHCN guide supports listing/filtering by reference, but exported presence/stability is unproven | **unknown** |
...
The ACB ONE standard KHCN first-party guide explicitly documents a transaction-list workflow where the customer chooses **`Xuất file excel`** and saves the file. The same guide also documents listing transactions by `số tham chiếu`, which establishes that reference-based lookup exists in the UI/workflow but does not prove that the reference appears in the exported file or is stable across repeated/overlapping exports. Separate ACB store-management first-party material independently confirms Excel history/statement download and store-oriented output structure. ACB also documents transaction-history states in ACB ONE (`đã thực hiện`, `chờ xử lý`, `đặt lịch`).

These facts remain scoped. Consumer Excel artifact availability is now directly supported, but exact exported headers, `.xls` versus `.xlsx`, debit/credit conventions, status representation and provider-stable source identity are still not established.

Current decision: Excel availability is confirmed for the standard KHCN transaction-list scope; exact layout normalization and source identity remain disabled.
...
- ACB ONE standard KHCN user guide — transaction listing and `Xuất file excel`:
