...
4. Check for scope creep: anything in the diff that is not covered by the packet's Implementation/Tasks section is out of scope and must be flagged.
...
7. Check UI work reuses existing tokens/components (see `docs/design-system.md`) and covers the states the packet lists (loading, empty, populated, error, recovery, mobile/tablet/desktop, accessibility).
...
10. Run the gates relevant to the change (`npm run lint`, `npm run typecheck`, `npm run test`, and others from `AGENTS.md` §Verification as applicable) and report actual pass/fail — do not accept a claim that they pass without running them yourself.
...
- Do not silently fix issues yourself — report them. Fixing is the implementing agent's or the human owner's job.
...
- **Ready to merge** — every criterion has evidence and gates pass.
- **Not ready** — list exactly what is missing or failing, ranked by what blocks merge vs. what can follow in a later slice.
