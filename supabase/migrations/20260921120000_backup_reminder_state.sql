-- S1 backup reminder: record when the tenant last produced an archive, and
-- surface it to Home through the existing read bundle.
--
-- `export_user_archive` stays read-only on purpose — its no-DML property is
-- pgTAP-proven. The write happens in `createArchiveBackupAction` after the
-- archive validates, which is the only caller that can produce one.
--
-- `last_backup_at` is nullable with no backfill: null honestly means "no backup
-- recorded". The reminder falls back to `created_at` as its baseline so a new
-- account is not nagged.
--
-- Both timestamps are emitted as `::date` — the chip measures whole days, and
-- casting avoids the session-DateStyle timestamptz-to-json hazard documented
-- in 20260812000000_export_user_archive.sql.
--
-- `backup_state` is additive; the application parses it as optional, matching
-- the ledger_trust skew contract in 20260913103000.
--
-- This replacement also corrects the transaction-start bound from -45 to -62
-- days: since the month-shape/prior-window work the app legitimately sends a
-- start up to 61 days back (2d-1 on day d of the month), so the old bound
-- rejected the bundle on the 24th+ of every month and forced the multi-query
-- fallback for the entire last week.

alter table public.profiles
  add column last_backup_at timestamptz;

comment on column public.profiles.last_backup_at is
  'When this tenant last produced a complete archive backup. Written by the backup server action after the archive validates; null means never.';
...
comment on function public.get_dashboard_bundle(date, date, integer) is
  'Bounded RLS-aware dashboard read model; one Data API call per render, including known-ledger trust state and backup recency without asserting external source completeness.';
