-- Emit `savings_goals.created_at` in the dashboard bundle.
--
-- `countGoalPaceAttention` (added with the goal pace work) anchors each goal's
-- linear pace expectation on `createdAt`, which `mapGoalRow` reads from the
-- `created_at` field. The dedicated `/goals` read (`getGoalsWorkspace`) already
-- selects it, but the bundle's goals projection did not — so on the primary
-- one-RPC dashboard path every goal arrived with `createdAt: null` and the
-- attention chip could only count *overdue* goals, never behind-pace ones.
--
-- The addition is additive-safe: the function returns jsonb, and the zod
-- schema already accepts `created_at` as nullish, so application deploys can
-- run ahead of or behind this migration without a parse failure. The raw
-- timestamptz is emitted (same implicit cast as `transactions[].created_at`
-- above) so the Vietnam-day conversion stays in `vietnamDay` — casting to
-- `::date` here would bake in the database session's timezone, not Asia/
-- Ho_Chi_Minh.
--
-- Body is identical to the 20260922120000 definition except the new field.
