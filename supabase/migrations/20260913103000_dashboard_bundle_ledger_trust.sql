-- #590: surface the existing deterministic ledger trust contract on Home without
-- reintroducing dashboard Data API fan-out. This keeps get_dashboard_bundle as
-- the single authenticated request and composes the already-reviewed
-- SECURITY INVOKER ledger_trust_summary() result into its JSON response.
--
-- The application treats ledger_trust as optional so code remains compatible
-- with an older database during application-before-migration deployment skew.
...
comment on function public.get_dashboard_bundle(date, date, integer) is
  'Bounded RLS-aware dashboard read model; one Data API call per render, including known-ledger trust state without asserting external source completeness.';
