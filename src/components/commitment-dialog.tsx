...
export function CommitmentDialog({ open, commitment, accounts, categories, onClose, onSave }: {
  ...
  return <dialog ref={ref} className="account-dialog commitment-dialog" onCancel={(event) => { event.preventDefault(); if (!submitting) onClose(); }} aria-labelledby="commitment-dialog-title">
    ...
    <form className="account-form" onSubmit={submit}>
      ...
      <label><span>Số tiền mỗi tháng</span><div className="account-money-input"><input name="amount" inputMode="numeric" defaultValue={commitment ? formatMoneyInput(String(commitment.amount)) : ""} placeholder="0" onInput={(event) => { event.currentTarget.value = formatMoneyInput(event.currentTarget.value); setError(""); }} /><strong>₫</strong></div></label>
