...
export function BudgetDialog({ open, budget, categories, monthStart, onClose, onSave }: {
  ...
  return <dialog ref={dialogRef} className="account-dialog budget-dialog" onCancel={(event) => { event.preventDefault(); if (!submitting) onClose(); }} onClose={onClose} aria-labelledby="budget-dialog-title">
    ...
    <form className="account-form" onSubmit={handleSubmit}>
      ...
      <label><span>Hạn mức tháng</span><div className="account-money-input"><input name="limit" inputMode="numeric" defaultValue={budget ? formatMoneyInput(String(budget.limit)) : ""} placeholder="0" onInput={(event) => { event.currentTarget.value = formatMoneyInput(event.currentTarget.value); setError(""); }} autoFocus={Boolean(budget)} /><strong>₫</strong></div></label>
