...
export function EditTransactionDialog({ transaction, accounts, categories, onClose, onSave, disabled = false }: {
  ...
  return <dialog ref={ref} className="transaction-dialog edit-transaction-dialog" onCancel={(event) => { event.preventDefault(); if (!submitting) onClose(); }} onClose={onClose} aria-labelledby="edit-transaction-title">
    ...
    <form onSubmit={submit}>
      ...
      <label className="amount-field"><span>{isTransfer ? "Số tiền chuyển" : "Số tiền"}</span><div><input autoFocus inputMode="numeric" autoComplete="off" value={amount} onChange={(event) => { setAmount(formatMoneyInput(event.target.value)); changed(); }} /><strong>₫</strong></div></label>
      ...
      <button className="primary-button dialog-submit" type="submit" disabled={disabled || submitting || accounts.length < (isTransfer ? 2 : 1) || (!isTransfer && !availableCategories.length)}><Icon name="check" />{submitting ? "Đang cập nhật..." : "Lưu thay đổi"}</button>
    </form>
  </dialog>;
}
