...
export async function createArchiveBackupAction(): Promise<BackupActionResult> {
  ...
  // needed. Accept every historical v1 generation plus the current payee
  // generation through one fail-closed wrapper over the proven R5 validator.
  const validated = validateMoneyFlowArchiveWithPayee(data);
  if (!validated.ok) {
...
export async function restoreArchiveAction(
  ...
  const validated = validateMoneyFlowArchiveWithPayee(archive);
