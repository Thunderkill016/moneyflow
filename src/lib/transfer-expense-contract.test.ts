...
test("TASK-253: transfer amount not in topExpenseCategories (category spend)", () => {
  ...
  if (foodSample > 0) assert.equal(food?.amount, foodSample);
  assert.ok(!topWith.some((row) => row.amount >= TRANSFER_AMOUNT));
});
...
test("TASK-253: transfer amount not in report category spend or expense total", () => {
  ...
  assert.ok(!reportWith.categories.some((c) => c.name === "Chuyển tiền" || c.amount === TRANSFER_AMOUNT));
  assert.ok(reportWith.totals.transactions >= reportWithout.totals.transactions);
  assert.equal(
    reportWith.totals.expense,
    pureExpenseTotal(
      demoLedger.filter((r) => r.occurredOn >= range.currentStart && r.occurredOn <= range.currentEnd),
      MONTH,
    ),
  );
});
