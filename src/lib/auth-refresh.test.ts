...
test("authentication preserves Google, password and recovery controls", () => {
  ...
  assert.match(authForm, /type=\{passwordVisible \? "text" : "password"\}/);
});
...
test("auth copy stays factual and task-focused", () => {
  ...
  assert.match(authForm, /title: "Mở lại sổ của bạn"/);
  assert.match(authForm, /submit: "Đăng nhập"/);
  assert.match(authForm, /aria-label="Giới thiệu MoneyFlow"/);
});

test("authentication is form-first, responsive, themed and motion accessible", () => {
  assert.match(authStyles, /min-height:\s*100svh/);
  assert.match(authStyles, /\.shell\b/);
  assert.match(authStyles, /\.contextPanel\b/);
  ...
  assert.match(authStyles, /min-height:\s*52px/);
