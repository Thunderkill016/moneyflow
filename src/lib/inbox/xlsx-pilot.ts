...
const PILOT_DEBIT_HEADERS =
  /^(debit|nợ|ghi\s*no|ghi\s*nợ|so\s*tien\s*ghi\s*no|số\s*tiền\s*ghi\s*nợ|so\s*tien\s*rut\s*ra|số\s*tiền\s*rút\s*ra)$/i;
const PILOT_CREDIT_HEADERS =
  /^(credit|có|ghi\s*co|ghi\s*có|so\s*tien\s*ghi\s*co|số\s*tiền\s*ghi\s*có|so\s*tien\s*gui\s*vao|số\s*tiền\s*gửi\s*vào)$/i;
 ...
 * the generic fallback pick a balance/reference column. Observed amount-qualified
 * debit/credit and withdraw/deposit headers are also treated as debit/credit roles
 * without introducing bank-owned financial logic. A separate standalone direction
 * column can qualify an otherwise unsigned amount, but it never becomes identity
 * or a bank-specific contract. The inspection object contains structural metadata
 * only; no cell text, amount, description, account number, sheet name or raw row
 * is returned.
 */
