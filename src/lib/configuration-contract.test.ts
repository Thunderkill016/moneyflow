...
const supabaseConfigSource = readFileSync("src/lib/supabase/config.ts", "utf8");
...
test("runtime configuration code contains no project-specific deployment hostname", () => {
...
test("demo mode is explicit instead of inferred from placeholder strings", () => {
  assert.match(supabaseConfigSource, /process\.env\.NEXT_PUBLIC_APP_MODE/);
  assert.doesNotMatch(
    supabaseConfigSource,
    /PLACEHOLDER_URLS|PLACEHOLDER_KEYS|replace_me|placeholder/i,
  );
  assert.match(supabaseConfigSource, /SupabaseConfigurationError/);
});
