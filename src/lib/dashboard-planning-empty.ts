/**
 * R3 — Dashboard ledger empty + planning-card empty contracts.
...
/** Full-page ledger empty on Dashboard (EmptyState). */
...
export const PLANNING_EMPTY_WEEKLY_WEEK: PlanningCardEmptyConfig = {
  ...
  actionLabel:
    WEEKLY_SUMMARY_REPORTS_LABEL.replace(/\s*→\s*$/, "").trim() ||
    WEEKLY_SUMMARY_REPORTS_LABEL,
  actionHref: WEEKLY_SUMMARY_REPORTS_HREF,
};

/** Exactly one primary action (or none for progressive empty). */
export function planningEmptyActionCount(
  config: PlanningCardEmptyConfig,
): number {
